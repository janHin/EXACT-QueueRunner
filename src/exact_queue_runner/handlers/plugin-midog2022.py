...
class MIDOG22Inference(DetectionInference):
    ...
    def configure_model(self):
        ...
        modelpath = os.path.join('QueueRunner', 'handlers', 'checkpoints', 'midog22.pth')
        ...
        model = RetinaNetDA(encoder, n_classes=2, n_anchors=len(scales) * len(ratios),sizes=[size[0] for size in sizes], chs=128, final_bias=-4., n_conv=3, imsize=(512,512), n_domains=6)
...
def inference(apis:dict, job:PluginJob, update_progress:Callable, **kwargs):
        ...
        try:
            ...
            inference_module = MIDOG22Inference(fname = tpath, update_progress = update_progress)
